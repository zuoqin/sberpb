...
The `home-routes` handler in the `sberstatic.routes.home` namespace
...
The `home-page` function will in turn call the `sberstatic.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `sberstatic.handler` namespace:
...
Request middleware functions are located under the `sberstatic.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `sberstatic.dev-middleware` namespace found in
