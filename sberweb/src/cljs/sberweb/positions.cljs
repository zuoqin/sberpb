...
(defn getPositions [] 
  ...
  (if (> (count (:trips ((keyword (:selecteduser @sbercore/app-state)) @sbercore/app-state)) 0))
    (sbercore/setUsersDropDown)
    (GET (str settings/apipath "api/trip?login=" (:login (:user @sbercore/app-state)) ) {
...
(defcomponent showpositions-view [data owner]
  (render
    ...
    (dom/div {:className "list-group" :style {:display "block"}}
      (map (fn [item]
        (dom/span
          (dom/a {:className "list-group-item" :href (str  "#/postrans/" (nth item 0) ) }
            (dom/h4  #js {:className "list-group-item-heading" :dangerouslySetInnerHTML #js {:__html (nth item 1)}} nil)

            (dom/h4 {:className "list-group-item-heading"} "jkghjkhkj"   )
	...
	(:positions ((keyword (:selecteduser @sbercore/app-state)) @data) )
      )
    )
  )
)
