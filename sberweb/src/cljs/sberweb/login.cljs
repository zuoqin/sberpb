...
(defn OnGetPositions [response]
  (swap! sbercore/app-state assoc-in [(keyword (:login (:user @sbercore/app-state))) :positions]   response )
  (aset js/window "location" "#/positions")
)


(defn reqpositions []
  (GET (str settings/apipath "api/position?client=" (:client (:user @sbercore/app-state )) ) {:handler OnGetPositions
